...
no.of.tweets <- 10000
  ...
  twitter_function<-function(hashtag,df) {
  ...
  hashtag_total <- hashtag_df%>%
                        mutate(week=week(Sys.Date()))%>%
                        summarise(hashtag_count=n())
} else{
  hashtag_one_day <- hashtag_df%>%
                        mutate(week=week(Sys.Date()))%>%
                        summarise(late_night_count=n())
  
  hashtag_total <- rbind(hashtag_total, hashtag_one_day) %>%
                      group_by(week)
}
    return(hashtag_total,df)
    }
